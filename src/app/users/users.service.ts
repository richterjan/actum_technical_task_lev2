...
export class UsersService {
  ...
  private locationTerm: string;
  private languageTerm: string;
  public perPage: number;
  public page: number;

  users: Subject<UserResponse> = new ReplaySubject<UserResponse>(1);
  locations: Subject<any> = new ReplaySubject<any>(1);
  ...
  getUsers() {
    ...
    if (searchString.length > 0) {
      ...
      this.http.get<UserResponse>(this.usersUrl, {headers, params}).pipe(
  ...
  getLocations() {
    const result = Object(cities).filter((city: any) => city.name.toLowerCase().includes(this.locationTerm.toLowerCase()));
